...
class OurArguments(TrainingArguments):
    ...
    load_int8: bool = True  # load model parameters as int8
    ...
    perturb_scaling_factor: int = 4
    bernperturb: bool = True
    quantperturb: bool = False
...
class Framework:
    ...
    def train(self, train_samples, eval_samples):
        ...
        if self.args.quantize == True and self.args.smooth == False and self.args.qllm == False:
            set_quantizer(self.args)
            quant_model = quantize_model(self.model)
        ...
        elif self.args.quantize == True and self.args.smooth == True:
            set_quantizer(self.args)
            quant_model = smoothquantize_model(self.model)
            enable_quantization(quant_model)
            print(quant_model)
            # if self.args.trainer != None:
            #     for name, param in quant_model.named_parameters():
        ...
        elif self.args.quantize == True and self.args.qllm == True:
            set_quantizer(self.args)
            quant_model = qllmquantize_model(self.model)
            enable_quantization(quant_model)
            print(quant_model)
            # if self.args.trainer != None:
            #     for name, param in quant_model.named_parameters():

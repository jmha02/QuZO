...
def perturb_quant(x: torch.tensor, bit=8):
        min_value = x.min()
        max_value = x.max()
        num_levels = 2**bit
        scaled_data = (x - min_value) / (max_value - min_value) * (num_levels - 1)
        quantized_data = torch.round(scaled_data)                                 
        dequant_data = quantized_data / (num_levels - 1) * (max_value - min_value) + min_value
        # if min_value != max_value:
        #  plot_hist(dequant_data,module_name="int_pertub")
...
class OurTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        if args.distributed and args.delayed_start:
            print("Local rank {} Go to sleep".format(args.local_rank))
            time.sleep((args.local_world_size - 1 - args.local_rank) * 60)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                elif args.trainer == "zo_ours":
                    if (((step + 1) % args.gradient_accumulation_steps == 1)) :
                            self.zo_random_seed = np.random.randint(1000000000)
                    #         print("setting seed")
                    # print("zo step random seed ",self.zo_random_seed)    
                    tr_loss_step = self.zo_step_grad_acc(model, inputs) 
                    self.grad_acc.append(self.projected_grad)  
                else:
                    if (
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        perturb_scaling=self.args.perturb_scaling_factor
        for name, param in self.named_parameters_to_optim:
            z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)  
            param.data = param.data + scaling_factor * z * self.args.zo_eps

    def zo_perturb_parameters_binary(self, random_seed=None, scaling_factor=1):
        """
        Perturb the parameters with random vector z.
        Input: 
        - random_seed: random seed for MeZO in-place perturbation (if it's None, we will use self.zo_random_seed)
        ...
        perturb_scaling=self.args.perturb_scaling_factor
        for name, param in self.named_parameters_to_optim:
            if self.args.bernperturb:
                z = perturb_scaling*torch.bernoulli(0.5*torch.ones(size=param.data.size(), device=param.data.device, dtype=torch.int8))
            else:
                z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)  
            if self.args.quantperturb:
                temp = perturb_quant(z,bit=16)
                z=temp
            param.data = param.data + scaling_factor * z * self.args.zo_eps
    ...
    def zo_step_grad_acc(self, model, inputs):
        """
        Estimate gradient by MeZO. Return the loss from f(theta + z)
        """
        args = self.args
        ...
        #             param.grad = (self.projected_grad * z + args.weight_decay * param.data) / self.args.gradient_accumulation_steps
        #         else:
        #             param.grad += (self.projected_grad * z + args.weight_decay * param.data) / self.args.gradient_accumulation_steps
    ...
    def zo_update_v1(self, model):
        ...
        # Reset the random seed for sampling zs
        torch.manual_seed(self.zo_random_seed)  
        # print("zo update random seed",self.zo_random_seed)   
        perturb_scaling=self.args.perturb_scaling_factor   
        # Gradient Accumulation
        total_acc = 0
        acc_array = torch.tensor(self.grad_acc)    
        for value in acc_array:
            total_acc += value
        
        projected_grad = total_acc / args.gradient_accumulation_steps

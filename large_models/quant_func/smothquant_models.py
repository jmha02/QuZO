...
import numpy as np
import copy
from .smothquant_modules import SmoothLinearQuantizer
...
def smoothquantize_model(model):
    """
    ...
    if type(model) == nn.Linear:
        quant_mod = SmoothLinearQuantizer(**quant_args)
        quant_mod.set_param(model)
        return quant_mod
    elif type(model) == nn.Sequential:
        mods = []
        for n, m in model.named_children():
            mods.append(smoothquantize_model(m))
        ...
        for attr in dir(model):
            ...
            if isinstance(mod, nn.Module) and attr != 'base_model' and attr!= 'lm_head':
                setattr(q_model, attr, smoothquantize_model(mod))
        return q_model

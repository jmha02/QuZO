#####
# Trainer for Zeroth-order Fine-tuning
# Edit based on transformers=4.46.1
#####
...
class OurTrainer(Trainer):
    ...
    def zo_lowbitperturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            # fp_perturb = torch.empty_like(param.data).uniform_(0, 1)
                # print(name)
            # quantized_data, scaling_data, zero_data = zo_quant_data(param.data,nbits=self.args.wbit,stochastic=False, blk_exp=False,sym=False)
            if self.args.quantized_perturb_ours == True:
                ...
                # self.log({f"quzo/{name}_2_perturbation_low-bit-mean": quantized_perturb.mean().item()})
                # self.log({f"quzo/{name}_2_perturbation_quant_diff-mean": (fp_perturb-dequantized_perturb).mean().item()})
                # self.log({f"quzo/{name}_2_perturbation_low-bit-std": quantized_perturb.std().item()})
                # self.log({f"quzo/{name}_2_perturbation_quant_diff-std": (fp_perturb-dequantized_perturb).std().item()})
            else:
                quantized_perturb, s, z = zo_quant(fp_perturb,nbits=self.args.perturb_bits,stochastic=True)
                dequantized_perturb = zo_dequant(quantized_perturb,s ,z) 
                # self.log({f"quzo/{name}_1_perturbation_low-bit-mean": quantized_perturb.mean().item()})
                # self.log({f"quzo/{name}_1_perturbation_quant_diff-mean": (fp_perturb-dequantized_perturb).mean().item()})
                # self.log({f"quzo/{name}_1_perturbation_quant_diff-std": quantized_perturb.std().item()})
                # self.log({f"quzo/{name}_1_perturbation_quant_diff-std": (fp_perturb-dequantized_perturb).std().item()})
            # mask = torch.rand_like(quantized_perturb) >= (0 / 100)
            # dequantized_perturb = dequantized_perturb 
            # quantized_add = quantized_data + scaling_factor*dequantized_perturb*5
            # dequant_data = zo_dequant(quantized_data,scaling_data ,zero_data)
            # error_mean = torch.mean(param.data)-torch.mean(dequant_data)
            # print(error_mean,name)
            # param.data = dequant_data
                
            param.data = param.data + scaling_factor * dequantized_perturb * self.args.zo_eps
            # dequant_data = zo_dequant(quantized_data,scaling_data ,zero_data)


            # bi_refer = torch.bernoulli(0.5*torch.ones(size=param.data.size(), device=param.data.device, dtype=torch.int8))
            # param.data = param.data + scaling_factor * bi_refer * self.args.zo_eps
            random_seed += 2
    ...
    def zo_dist_step(self, model, inputs):
        """
        ...
        # build distributed loop
        rank = dist.get_rank()
        world_size = dist.get_world_size()
        iterations_per_gpu = (self.args.num_pertub + world_size - 1) // world_size  # Ceiling division
        start = rank * iterations_per_gpu
        end = min(start + iterations_per_gpu, self.args.num_pertub)
        ...
        for i in range(start, end):
            ...
            self.zo_perturb_parameters(scaling_factor=1, random_seed=zo_random_seed)
        all_projected_grad_list = [None] * world_size
        all_zo_random_seed_list = [None] * world_size
        dist.all_gather_object(all_projected_grad_list, projected_grad_list)
        dist.all_gather_object(all_zo_random_seed_list, zo_random_seed_list)
        self.zo_random_seed = all_zo_random_seed_list
        self.projected_grad = all_projected_grad_list
        # No gradient accumulation support
        # assert self.args.gradient_accumulation_steps == 1
        # print(f'len check {len(self.zo_random_seed)} {len(self.projected_grad)}')
        pdb.set_trace()
        return loss1
    ...
    def lowbit_zo_step(self, model, inputs):
        ...
        # self.projected_grad, scale, zero = zo_quant(self.projected_grad ,nbits=4,stochastic=False,sym=False)
    ...
    def zo_update(self, model):
        ...
        for i in range(iterations):
            ...
            for name, param in self.named_parameters_to_optim:
                ...
                z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device,
                                 dtype=param.data.dtype)
                grad_tmp[name] += self.projected_grad[i] * z
        for name, param in self.named_parameters_to_optim:
            # param.data = param.data - self._get_learning_rate() * grad_tmp[name] / iterations
            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                param.data = param.data - self._get_learning_rate() * (
                        grad_tmp[name] / iterations + args.weight_decay * param.data)
            else:
                param.data = param.data - self._get_learning_rate() * grad_tmp[name] / iterations
        self.lr_scheduler.step()
    ...
    def lowbit_zo_update(self, model):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            # if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
            #     self.projected_grad,scale, zero =  zo_quant(self.projected_grad ,nbits=1,stochastic=False,sym=False)
            #     self.projected_grad = zo_dequant(self.projected_grad,scale, zero)
            ...
            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                ...
                param.data = param.data - self._get_learning_rate() * (self.projected_grad * z + args.weight_decay * param.data)
                quantized_data, scaling, zero = zo_quant_data(param.data,nbits=8,stochastic=True, sym=True)
